...
async def save_call_recording(audio_data: bytes, call_sid: str, from_number: str = None, to_number: str = None) -> str:
    ...
    try:
        ...
        # __file__ is sambanova/twilio_handler.py
...
async def twilio_handler(websocket):
    ...
    Manages audio streams and interaction with the Sambanova agent.
    """
    ...
    try:
        # Get the directory where this file is located (sambanova)
        ...
        logger.error(f"❌ Error initializing Sambanova agent: {e}")
        import traceback
        logger.error(f"Traceback: {traceback.format_exc()}")
        # Create a simple fallback agent without MCP tools
        agent_graph = TodoAgent(tools=[]).build_graph()
        logger.info("⚠️ Using fallback Sambanova agent without MCP tools")
    ...
    try:
        initial_input = {
            "messages": [
                HumanMessage(content="You have just received a phone call. Briefly introduce yourself as the Sambanova productivity assistant, and ask how you can help.")
            ]
        }
        ...
        try:
            ...
            if event == "start":
                ...
                try:
                    ...
                    # 2. Get response from the Sambanova agent
                    agent_input = {"messages": [HumanMessage(content=transcribed_text)]}
                    agent_response_text = ""
                    
                    print("--- Sambanova Assistant ---\n")

...
# Initialize OpenAI clients for Sambanova
...
async def record_audio_until_stop():
    ...
    filename = f"sambanova_audio_{timestamp}.wav"
    ...
    # Note: Sambanova may not support Whisper, using OpenAI for transcription
...
async def transcribe_audio_bytes(audio_bytes: bytes) -> str:
    ...
    try:
        ...
        wav_buffer.seek(0)
        wav_buffer.name = "sambanova_streamed_audio.wav"
        
        # Use OpenAI for transcription (Sambanova may not support Whisper)
        openai_for_transcription = AsyncOpenAI()  # Use default OpenAI for transcription
        transcription = await openai_for_transcription.audio.transcriptions.create(
            model="whisper-1",
            file=wav_buffer,
...
async def play_audio(message: str):
    """Plays the audio response from the Sambanova agent."""
    cleaned_message = message.replace("**", "")

    # Note: Using OpenAI TTS since Sambanova may not support speech synthesis
    # In production, you would use Sambanova's TTS if available

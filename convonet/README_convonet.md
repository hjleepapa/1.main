# Convonet Todo Management System

A comprehensive productivity management system powered by Convonet AI, featuring voice and web interfaces for task management, reminders, and calendar integration.

## Features

- **Natural Language Processing**: Powered by Convonet AI for intelligent task understanding
...
- **Database Management**: PostgreSQL with dedicated convonet tables
...
- **PostgreSQL**: Primary database with dedicated convonet schema
- **MCP Server**: Database operations through Model Context Protocol

### AI Integration
- **Convonet Models**: Core natural language processing
...
### Tables
- `todos_convonet`: Main todo items with priority and due dates
- `reminders_convonet`: Reminder items with importance levels  
- `calendar_events_convonet`: Calendar events synced with Google Calendar
- `call_recordings_convonet`: Voice call recordings and transcriptions
...
### Web Interface
- `GET /convonet_todo/`: Main web interface
- `POST /convonet_todo/run_agent`: Process natural language commands

### Voice Interface
- `POST /convonet_todo/twilio/call`: Handle incoming voice calls
- `POST /convonet_todo/twilio/process_audio`: Process voice input
...
- `CONVONET_API_KEY`: API key for Convonet AI services
...
### Web Interface
1. Navigate to `/convonet_todo/`
2. Use the web interface or API endpoints
3. Send JSON requests to `/convonet_todo/run_agent`

### Voice Interface
1. Configure Twilio webhook to point to `/convonet_todo/twilio/call`
2. Call the configured phone number
3. Speak naturally to create and manage todos

### API Usage
```bash
curl -X POST /convonet_todo/run_agent \
...
├── assistant_graph_todo.py     # Convonet AI agent configuration

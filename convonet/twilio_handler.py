...
async def save_call_recording(audio_data: bytes, call_sid: str, from_number: str = None, to_number: str = None) -> str:
    ...
    try:
        ...
        # __file__ is convonet/twilio_handler.py
...
async def twilio_handler(websocket):
    ...
    Manages audio streams and interaction with the Convonet agent.
    """
    ...
    try:
        # Get the directory where this file is located (convonet)
        ...
        try:
            ...
            logger.info("✅ Convonet agent and tools initialized successfully")
        finally:
            os.chdir(original_cwd)
    except Exception as e:
        logger.error(f"❌ Error initializing Convonet agent: {e}")
        import traceback
        logger.error(f"Traceback: {traceback.format_exc()}")
        # Create a simple fallback agent without MCP tools
        agent_graph = TodoAgent(tools=[]).build_graph()
        logger.info("⚠️ Using fallback Convonet agent without MCP tools")
    ...
    try:
        initial_input = {
            "messages": [
                HumanMessage(content="You have just received a phone call. Briefly introduce yourself as the Convonet productivity assistant, and ask how you can help.")
            ]
        }
        ...
        try:
            ...
            if event == "start":
                ...
                try:
                    ...
                    # 2. Get response from the Convonet agent
                    agent_input = {"messages": [HumanMessage(content=transcribed_text)]}
                    agent_response_text = ""
                    
                    print("--- Convonet Assistant ---\n")
                    ...
                    if agent_response_text:
                        logger.info("Sending Convonet agent response to Twilio...")

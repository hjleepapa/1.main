...
def run_migration():
    ...
    try:
        ...
        with engine.connect() as conn:
            conn.execute(text("""
                CREATE TABLE IF NOT EXISTS users_convonet (
            ...
            conn.execute(text("CREATE INDEX IF NOT EXISTS idx_users_email ON users_convonet(email);"))
            conn.execute(text("CREATE INDEX IF NOT EXISTS idx_users_username ON users_convonet(username);"))
            
            # Create teams table
            conn.execute(text("""
                CREATE TABLE IF NOT EXISTS teams_convonet (
                    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
            ...
            conn.execute(text("""
                CREATE TABLE IF NOT EXISTS team_memberships_convonet (
                    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
                    team_id UUID NOT NULL REFERENCES teams_convonet(id),
                    user_id UUID NOT NULL REFERENCES users_convonet(id),
            ...
            conn.execute(text("CREATE INDEX IF NOT EXISTS idx_team_memberships_team_id ON team_memberships_convonet(team_id);"))
            conn.execute(text("CREATE INDEX IF NOT EXISTS idx_team_memberships_user_id ON team_memberships_convonet(user_id);"))
            conn.commit()
        ...
        with engine.connect() as conn:
            result = conn.execute(text("""
                ...
                WHERE table_name = 'todos_convonet' 
            ...
            if 'creator_id' not in existing_columns:
                conn.execute(text("ALTER TABLE todos_convonet ADD COLUMN creator_id UUID;"))
                conn.execute(text("CREATE INDEX IF NOT EXISTS idx_todos_creator_id ON todos_convonet(creator_id);"))
            
            if 'assignee_id' not in existing_columns:
                conn.execute(text("ALTER TABLE todos_convonet ADD COLUMN assignee_id UUID;"))
                conn.execute(text("CREATE INDEX IF NOT EXISTS idx_todos_assignee_id ON todos_convonet(assignee_id);"))
            
            if 'team_id' not in existing_columns:
                conn.execute(text("ALTER TABLE todos_convonet ADD COLUMN team_id UUID;"))
                conn.execute(text("CREATE INDEX IF NOT EXISTS idx_todos_team_id ON todos_convonet(team_id);"))
            
            if 'is_private' not in existing_columns:
                conn.execute(text("ALTER TABLE todos_convonet ADD COLUMN is_private BOOLEAN DEFAULT FALSE NOT NULL;"))
        ...
        with SessionLocal() as session:
            ...
            admin_user = session.query(User).filter(User.email == 'admin@convonet.com').first()
        ...
        print("\n📋 Sample credentials:")
        print("Email: admin@convonet.com")

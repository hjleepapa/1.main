...
# Initialize OpenAI clients for Convonet
openai_async = AsyncOpenAI(
    api_key=os.getenv("CONVONET_API_KEY"),
    base_url="https://api.convonet.ai/v1"
)
openai = OpenAI(
    api_key=os.getenv("CONVONET_API_KEY"),
    base_url="https://api.convonet.ai/v1"
...
async def record_audio_until_stop():
    ...
    filename = f"convonet_audio_{timestamp}.wav"
    ...
    # Note: Convonet may not support Whisper, using OpenAI for transcription
    openai_for_transcription = AsyncOpenAI()  # Use default OpenAI for transcription
    transcription = await openai_for_transcription.audio.transcriptions.create(
        model="whisper-1",
        file=audio_bytes,
    )
...
async def transcribe_audio_bytes(audio_bytes: bytes) -> str:
    ...
    try:
        ...
        wav_buffer.seek(0)
        wav_buffer.name = "convonet_streamed_audio.wav"
        
        # Use OpenAI for transcription (Convonet may not support Whisper)
        openai_for_transcription = AsyncOpenAI()  # Use default OpenAI for transcription
        transcription = await openai_for_transcription.audio.transcriptions.create(
            model="whisper-1",
            file=wav_buffer,
...
async def play_audio(message: str):
    """Plays the audio response from the Convonet agent."""
    cleaned_message = message.replace("**", "")

    # Note: Using OpenAI TTS since Convonet may not support speech synthesis
    # In production, you would use Convonet's TTS if available
    openai_for_tts = AsyncOpenAI()  # Use default OpenAI for TTS
    
    async with openai_for_tts.audio.speech.with_streaming_response.create(

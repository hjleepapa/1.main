...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'simple_audio_player_secret'
...
def index():
    """Main page"""
    return render_template('simple_audio_player.html')
...
@app.route('/api/session/<session_id>/analyze')
...
def handle_connect():
    ...
    print(f"✅ Simple audio player client connected: {request.sid}")
    emit('connected', {'message': 'Connected to simple audio player'})
